...
人类说话。瞄准 agent 的破坏性动词命中 HIGH；同一个动词若没有 agent 收件人
（「删掉 `node_modules` 重试」）则降级处理，让无害的开发者 prose 不会淹没你的 CI 日志。
...
| `rules`      | rule 列表      | —    | 每条含 `id`、`severity`、`verbs[]`、可选 `addressees[]`、`description`。 |
